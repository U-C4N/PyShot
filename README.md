...
The pure helpers (`sel_box`, `next_free_path`, `to_dib`, `parse_hotkey`, `_window_at`) are deliberately free of Win32 and tkinter so they can be tested headlessly; CI runs them on `windows-latest`, which is required rather than preferred because the module loads `user32`/`kernel32` and imports `win32clipboard` at import time.
